  ...
  <div class="d-flex" id="wrapper">
    ...
    <div id="page-content-wrapper">
      ...
      <div class="container-fluid">
          <h2 class="mt-4">Overzicht bestellingen</h2>

          <table class="table">
              <thead class="thead-dark">
              <tr>
                  <th scope="col">#</th>
                  <th scope="col">Aangeschaft door</th>
                  <th scope="col">soort ticket</th>
              ...
              <tbody>
              <?php
              $sql = "SELECT * FROM bestellingen JOIN tickets ON bestellingen.ticket_id = tickets.ticket_id JOIN gebruiker ON bestellingen.klant_id = gebruiker.klant_id WHERE betaald=1";
              $query = mysqli_query($con, $sql);
              $result = mysqli_num_rows($query);
              if($result >= 1) {
              while($result = mysqli_fetch_assoc($query))  {
                  $orgDate = $result["datum"];
                  $newDate = date("j-n-Y H:i", strtotime($orgDate));
                  ?>
              <tr>
                  <th scope="row"><?php echo $result["bestelling_id"]; ?></th>
                  <td><?php echo $result["voornaam"]. " ". $result["tussenvoegsel"]. " " .$result["achternaam"]; ?></td>
                  <td><?php echo $result["naam"]; ?></td>
                  <td><?php echo $result["aantal"]; ?></td>
                  <td><?php echo "&euro;". $result["totaalprijs"]. ",-"; ?></td>
